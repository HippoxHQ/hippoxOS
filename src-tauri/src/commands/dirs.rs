use crate::commands::{
    get_app_root_dir, get_cache_dir, get_chart_dialog_history_dir, get_codeeditor_dialog_history_dir, get_data_dir, get_dialog_history_dir, get_external_cache_audio_dir, get_external_cache_sfx_dir, get_external_cache_sticker_dir, get_log_dir, get_map_dialog_history_dir, get_material_favorites_dir, get_notifications_dir, get_sandbox3d_dialog_history_dir, get_scheduled_tasks_dir, get_settings_dir, get_skill_history_dir, get_skills_dir, get_skills_market_dir, get_taskpool_dir, get_video_editing_system_dialog_history_dir,
};
use std::fs;
use std::path::Path;
...
pub fn save_scheduled_task(task_id: &str, data: &str) -> Result<String, String> {
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir).map_err(|e| format!("Failed to create scheduled tasks directory: {}", e))?;
    }
...
pub fn delete_scheduled_task(task_id: &str) -> Result<(), String> {
    ...
    if file_path.exists() {
        fs::remove_file(file_path).map_err(|e| format!("Failed to delete scheduled task: {}", e))?;
    }
    Ok(())
}
...
pub fn list_scheduled_task_ids() -> Result<Vec<String>, String> {
    ...
    let mut tasks = vec![];
    for entry in fs::read_dir(dir).map_err(|e| format!("Failed to read scheduled tasks dir: {}", e))? {
...
pub fn save_internal_setting(setting_dir: &Path, key: &str, data: &str) -> Result<String, String> {
    if !setting_dir.exists() {
        fs::create_dir_all(setting_dir).map_err(|e| format!("Failed to create setting directory: {}", e))?;
    }
...
pub fn load_internal_setting(setting_dir: &Path, key: &str) -> Result<Option<String>, String> {
    let file_path = setting_dir.join(format!("{}.json", key));
    if file_path.exists() {
        let content = fs::read_to_string(&file_path).map_err(|e| format!("Failed to read setting: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn init_default_session_if_empty() -> Result<(), String> {
    ...
    if !has_sessions {
        ...
        fs::create_dir_all(&session_dir).map_err(|e| format!("Failed to create session directory: {}", e))?;
        ...
        fs::write(&config_path, serde_json::to_string_pretty(&config).unwrap()).map_err(|e| format!("Failed to save config: {}", e))?;
        let chat_path = session_dir.join("chat.json");
        fs::write(&chat_path, serde_json::to_string_pretty(&welcome_message).unwrap()).map_err(|e| format!("Failed to save chat: {}", e))?;
