...
pub fn cmd_create_chart_dialog_session(
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir)
            .map_err(|e| format!("Failed to create chart dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !session_dir.exists() {
        fs::create_dir_all(&session_dir)
            .map_err(|e| format!("Failed to create chart session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
#[tauri::command]
pub fn cmd_load_chart_session_config(
    session_id: &str,
...
pub fn cmd_save_chart_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&chat_path, content)
        .map_err(|e| format!("Failed to save chart chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if config_path.exists() {
...
pub fn cmd_save_chart_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&terminal_path, content)
        .map_err(|e| format!("Failed to save chart terminal content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_chart_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if chat_path.exists() {
        let content = fs::read_to_string(&chat_path)
            .map_err(|e| format!("Failed to read chart chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_chart_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if terminal_path.exists() {
        let content = fs::read_to_string(&terminal_path)
            .map_err(|e| format!("Failed to read chart terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_chart_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&task_path, content)
        .map_err(|e| format!("Failed to save chart task content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_chart_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if task_path.exists() {
        let content = fs::read_to_string(&task_path)
            .map_err(|e| format!("Failed to read chart task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
#[tauri::command]
pub fn cmd_update_pinned_chart_sessions(
    session_id: String,
    pinned: bool,
