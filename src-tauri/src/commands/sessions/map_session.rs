...
pub fn cmd_update_pinned_map_sessions(
    ...
    let config_path = settings_dir.join("map_config.json");
    let mut config: serde_json::Value = if config_path.exists() {
        let content = fs::read_to_string(&config_path)
            .map_err(|e| format!("Failed to read map config: {}", e))?;
    ...
    let content = serde_json::to_string_pretty(&config)
        .map_err(|e| format!("Failed to serialize map config: {}", e))?;
    fs::write(&config_path, content).map_err(|e| format!("Failed to save map config: {}", e))?;
    Ok(pinned_sessions)
}
