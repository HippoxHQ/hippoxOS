...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(&config_path)
            .map_err(|e| format!("Failed to read map session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    let content = serde_json::to_string_pretty(&config)
        .map_err(|e| format!("Failed to serialize map session config: {}", e))?;
    fs::write(&config_path, content)
        .map_err(|e| format!("Failed to save map session config: {}", e))?;
    Ok(())
}
...
pub fn cmd_create_map_dialog_session(
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir)
            .map_err(|e| format!("Failed to create map dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !session_dir.exists() {
        fs::create_dir_all(&session_dir)
            .map_err(|e| format!("Failed to create map session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
pub fn cmd_list_map_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in
        fs::read_dir(dir).map_err(|e| format!("Failed to read map dialog history dir: {}", e))?
    {
...
pub fn cmd_delete_map_dialog_session(session_id: &str) -> Result<(), String> {
    ...
    if session_dir.exists() {
        fs::remove_dir_all(&session_dir)
            .map_err(|e| format!("Failed to delete map session: {}", e))?;
    }
...
pub fn cmd_save_map_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&chat_path, content)
        .map_err(|e| format!("Failed to save map chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if config_path.exists() {
...
pub fn cmd_save_map_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&terminal_path, content)
        .map_err(|e| format!("Failed to save map terminal content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_map_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if chat_path.exists() {
        let content = fs::read_to_string(&chat_path)
            .map_err(|e| format!("Failed to read map chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_map_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if terminal_path.exists() {
        let content = fs::read_to_string(&terminal_path)
            .map_err(|e| format!("Failed to read map terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_map_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&task_path, content)
        .map_err(|e| format!("Failed to save map task content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_map_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if task_path.exists() {
        let content = fs::read_to_string(&task_path)
            .map_err(|e| format!("Failed to read map task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
#[tauri::command]
pub fn cmd_update_pinned_map_sessions(
    session_id: String,
    pinned: bool,
