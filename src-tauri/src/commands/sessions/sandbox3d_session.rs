...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(&config_path)
            .map_err(|e| format!("Failed to read sandbox3d session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    let content = serde_json::to_string_pretty(&config)
        .map_err(|e| format!("Failed to serialize sandbox3d session config: {}", e))?;
    fs::write(&config_path, content)
        .map_err(|e| format!("Failed to save sandbox3d session config: {}", e))?;
    Ok(())
}
...
pub fn cmd_create_sandbox3d_dialog_session(
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir)
            .map_err(|e| format!("Failed to create sandbox3d dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !session_dir.exists() {
        fs::create_dir_all(&session_dir)
            .map_err(|e| format!("Failed to create sandbox3d session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
pub fn cmd_list_sandbox3d_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in
        fs::read_dir(dir).map_err(|e| format!("Failed to read sandbox3d dialog history dir: {}", e))?
    {
...
#[tauri::command]
pub fn cmd_load_sandbox3d_session_config(
    session_id: &str,
...
pub fn cmd_delete_sandbox3d_dialog_session(session_id: &str) -> Result<(), String> {
    ...
    if session_dir.exists() {
        fs::remove_dir_all(&session_dir)
            .map_err(|e| format!("Failed to delete sandbox3d session: {}", e))?;
    }
...
pub fn cmd_save_sandbox3d_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&chat_path, content)
        .map_err(|e| format!("Failed to save sandbox3d chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if config_path.exists() {
...
pub fn cmd_save_sandbox3d_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&terminal_path, content)
        .map_err(|e| format!("Failed to save sandbox3d terminal content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_sandbox3d_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if chat_path.exists() {
        let content = fs::read_to_string(&chat_path)
            .map_err(|e| format!("Failed to read sandbox3d chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_sandbox3d_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if terminal_path.exists() {
        let content = fs::read_to_string(&terminal_path)
            .map_err(|e| format!("Failed to read sandbox3d terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_sandbox3d_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&task_path, content)
        .map_err(|e| format!("Failed to save sandbox3d task content: {}", e))?;
    Ok(())
}
...
pub fn cmd_load_sandbox3d_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if task_path.exists() {
        let content = fs::read_to_string(&task_path)
            .map_err(|e| format!("Failed to read sandbox3d task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
#[tauri::command]
pub fn cmd_update_pinned_sandbox3d_sessions(
    session_id: String,
    pinned: bool,
