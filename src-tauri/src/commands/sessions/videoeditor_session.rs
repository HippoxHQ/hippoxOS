...
use crate::commands::{
    get_session_lock, get_settings_dir, get_video_editing_system_dialog_history_dir, load_metadata, load_session_config, load_session_metadata,
    save_session_config, save_session_metadata, save_session_tracks, update_session_track_stack, MaterialType, SessionMetadata, TrackTableMap,
};
...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(&config_path).map_err(|e| format!("Failed to read video session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    fs::write(&config_path, content).map_err(|e| format!("Failed to save video session config: {}", e))?;
    Ok(())
}
...
pub fn cmd_create_video_dialog_session(
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir).map_err(|e| format!("Failed to create video dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !session_dir.exists() {
        fs::create_dir_all(&session_dir).map_err(|e| format!("Failed to create video session directory: {}", e))?;
    }
    ...
    if !material_dir.exists() {
        fs::create_dir_all(&material_dir).map_err(|e| format!("Failed to create material directory: {}", e))?;
    }
    for sub_dir in ["videos", "audios", "images", "texts"] {
        let sub_path = material_dir.join(sub_dir);
        if !sub_path.exists() {
            fs::create_dir_all(&sub_path).map_err(|e| format!("Failed to create {} directory: {}", sub_dir, e))?;
        }
    }
...
pub fn cmd_list_video_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in fs::read_dir(dir).map_err(|e| format!("Failed to read video dialog history dir: {}", e))? {
...
pub fn cmd_delete_video_dialog_session(session_id: &str) -> Result<(), String> {
    ...
    let session_dir = dir.join(session_id);
    if session_dir.exists() {
        fs::remove_dir_all(&session_dir).map_err(|e| format!("Failed to delete video session: {}", e))?;
    }
...
pub fn cmd_save_video_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&chat_path, content).map_err(|e| format!("Failed to save video chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if config_path.exists() {
...
pub fn cmd_save_video_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&terminal_path, content).map_err(|e| format!("Failed to save video terminal content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_video_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if chat_path.exists() {
        let content = fs::read_to_string(&chat_path).map_err(|e| format!("Failed to read video chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_video_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if terminal_path.exists() {
        let content = fs::read_to_string(&terminal_path).map_err(|e| format!("Failed to read video terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_video_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&task_path, content).map_err(|e| format!("Failed to save video task content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_video_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if task_path.exists() {
        let content = fs::read_to_string(&task_path).map_err(|e| format!("Failed to read video task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_get_video_session_tracks(session_id: &str) -> Result<Vec<serde_json::Value>, String> {
    ...
    let content = fs::read_to_string(&metadata_path).map_err(|e| format!("Failed to read metadata: {}", e))?;
    let metadata: serde_json::Value = serde_json::from_str(&content).map_err(|e| format!("Failed to parse metadata: {}", e))?;
    let tracks =
        metadata.get("tracks").and_then(|t| t.as_object()).map(|obj| obj.values().cloned().collect::<Vec<serde_json::Value>>()).unwrap_or_default();
    Ok(tracks)
}
#[tauri::command]
