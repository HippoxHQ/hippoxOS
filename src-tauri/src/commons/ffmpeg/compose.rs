...
impl Ffmpeg {
    pub fn compose_tracks(&self, request: &ComposeRequest) -> Result<(), String> {
        ...
        let video_tracks: Vec<&TrackItem> = request.tracks.iter().filter(|t| t.track_type == "video").collect();
        let image_tracks: Vec<&TrackItem> = request.tracks.iter().filter(|t| t.track_type == "image").collect();
        let emoji_tracks: Vec<&TrackItem> = request.tracks.iter().filter(|t| t.track_type == "emoji").collect();
        let text_tracks: Vec<&TrackItem> = request.tracks.iter().filter(|t| t.track_type == "text").collect();
        let audio_tracks: Vec<&TrackItem> = request.tracks.iter().filter(|t| t.track_type == "audio").collect();
        let mut filter_parts = Vec::new();
        let mut inputs = Vec::new();
        let mut input_index = 0;
        let mut current_output = String::new();
        let total_duration = request.tracks.iter().map(|t| t.start + t.duration).fold(0.0, f64::max);
        if video_tracks.is_empty() && image_tracks.is_empty() && text_tracks.is_empty() && emoji_tracks.is_empty() {
            let bg_color = request.background_color.as_deref().unwrap_or("black");
            inputs.push(format!("-f lavfi -i color=c={}:s={}x{}:r={}:d={}", bg_color, request.width, request.height, request.fps, total_duration));
        ...
        for track in &image_tracks {
            ...
            let current_input = if current_output.is_empty() { format!("[0:v]") } else { current_output.clone() };
            filter_parts
                .push(format!("[{}:v]setpts=PTS-STARTPTS,scale={}:{},format=rgba,colorchannelmixer=aa={}[{}_scaled]", idx, w, h, opacity, label));
            let out_label = format!("o{}", idx);
            filter_parts.push(format!("{}[{}_scaled]overlay={}:{}[{}]", current_input, label, x, y, out_label));
        ...
        for (idx, track) in text_tracks.iter().enumerate() {
            ...
            let current_input = if current_output.is_empty() {
                let bg_color = request.background_color.as_deref().unwrap_or("black");
                inputs
                    .push(format!("-f lavfi -i color=c={}:s={}x{}:r={}:d={}", bg_color, request.width, request.height, request.fps, total_duration));
                format!("[0:v]")
            } else {
        ...
        if !audio_tracks.is_empty() {
            ...
            audio_filter =
                format!("{};{}amix=inputs={}:duration=longest[aout]", audio_inputs_str.join(";"), audio_labels.join(""), audio_tracks.len());
        }
        ...
        let output = Command::new("ffmpeg").args(&args).output().map_err(|e| format!("Failed to compose tracks: {}", e))?;
    ...
    pub fn overlay_media(
        ...
        let args = vec![
            ...
            format!("[1:v]{}[ov];[0:v][ov]overlay={}:{}:enable='between(t,{},{})'", scale_filter, x, y, start, start + duration),
            "-c:a".to_string(),
            "copy".to_string(),
            "-y".to_string(),
