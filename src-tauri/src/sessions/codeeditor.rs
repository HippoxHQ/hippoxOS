...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if FileUtils::path_exists(&config_path) {
        let content = FileUtils::read_file_to_string(&config_path).map_err(|e| format!("Failed to read codeeditor session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    let content = serde_json::to_string_pretty(&config).map_err(|e| format!("Failed to serialize codeeditor session config: {}", e))?;
    FileUtils::write_file_string(&config_path, &content).map_err(|e| format!("Failed to save codeeditor session config: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_create_codeeditor_dialog_session(
    ...
    if !FileUtils::path_exists(&dir) {
        FileUtils::ensure_dir(&dir).map_err(|e| format!("Failed to create codeeditor dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !FileUtils::path_exists(&session_dir) {
        FileUtils::ensure_dir(&session_dir).map_err(|e| format!("Failed to create codeeditor session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
pub fn cmd_list_codeeditor_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for path in FileUtils::read_dir(&dir).map_err(|e| format!("Failed to read codeeditor dialog history dir: {}", e))? {
        if path.is_dir() {
...
#[tauri::command]
pub fn cmd_load_codeeditor_session_config(session_id: &str) -> Result<Option<serde_json::Value>, String> {
    let dir = get_codeeditor_dialog_history_dir();
...
#[tauri::command]
pub fn cmd_update_codeeditor_session_config(session_id: &str, updates: String) -> Result<(), String> {
    let dir = get_codeeditor_dialog_history_dir();
    let session_dir = dir.join(session_id);
    let config_path = session_dir.join("config.json");
    if !FileUtils::path_exists(&config_path) {
...
pub fn cmd_save_codeeditor_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&chat_path, content).map_err(|e| format!("Failed to save codeeditor chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if FileUtils::path_exists(&config_path) {
...
pub fn cmd_save_codeeditor_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&terminal_path, content).map_err(|e| format!("Failed to save codeeditor terminal content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_codeeditor_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&chat_path) {
        let content = FileUtils::read_file_to_string(&chat_path).map_err(|e| format!("Failed to read codeeditor chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_codeeditor_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&terminal_path) {
        let content = FileUtils::read_file_to_string(&terminal_path).map_err(|e| format!("Failed to read codeeditor terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_codeeditor_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&task_path, content).map_err(|e| format!("Failed to save codeeditor task content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_codeeditor_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&task_path) {
        let content = FileUtils::read_file_to_string(&task_path).map_err(|e| format!("Failed to read codeeditor task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
#[tauri::command]
pub fn cmd_update_pinned_codeeditor_sessions(session_id: String, pinned: bool) -> Result<Vec<String>, String> {
