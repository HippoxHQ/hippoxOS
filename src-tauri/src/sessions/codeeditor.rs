...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(&config_path).map_err(|e| format!("Failed to read codeeditor session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    fs::write(&config_path, content).map_err(|e| format!("Failed to save codeeditor session config: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_create_codeeditor_dialog_session(
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir).map_err(|e| format!("Failed to create codeeditor dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !session_dir.exists() {
        fs::create_dir_all(&session_dir).map_err(|e| format!("Failed to create codeeditor session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
pub fn cmd_list_codeeditor_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in fs::read_dir(dir).map_err(|e| format!("Failed to read codeeditor dialog history dir: {}", e))? {
...
pub fn cmd_save_codeeditor_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&chat_path, content).map_err(|e| format!("Failed to save codeeditor chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if config_path.exists() {
...
pub fn cmd_save_codeeditor_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&terminal_path, content).map_err(|e| format!("Failed to save codeeditor terminal content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_codeeditor_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if chat_path.exists() {
        let content = fs::read_to_string(&chat_path).map_err(|e| format!("Failed to read codeeditor chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_codeeditor_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if terminal_path.exists() {
        let content = fs::read_to_string(&terminal_path).map_err(|e| format!("Failed to read codeeditor terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_codeeditor_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    fs::write(&task_path, content).map_err(|e| format!("Failed to save codeeditor task content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_codeeditor_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if task_path.exists() {
        let content = fs::read_to_string(&task_path).map_err(|e| format!("Failed to read codeeditor task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
