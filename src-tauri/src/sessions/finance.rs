...
fn get_pinned_sessions_from_config() -> Result<Vec<String>, String> {
    ...
    if FileUtils::path_exists(&config_path) {
        let content = FileUtils::read_file_to_string(&config_path).map_err(|e| format!("Failed to read chart session config: {}", e))?;
...
fn save_pinned_sessions_to_config(pinned_sessions: &[String]) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&config_path, &content).map_err(|e| format!("Failed to save chart session config: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_create_chart_dialog_session(
    ...
    if !FileUtils::path_exists(&dir) {
        FileUtils::ensure_dir(&dir).map_err(|e| format!("Failed to create chart dialog history directory: {}", e))?;
    }
    let session_dir = dir.join(session_id);
    if !FileUtils::path_exists(&session_dir) {
        FileUtils::ensure_dir(&session_dir).map_err(|e| format!("Failed to create chart session directory: {}", e))?;
    }
    let config = serde_json::json!({
...
pub fn cmd_list_chart_dialog_sessions() -> Result<Vec<serde_json::Value>, String> {
    ...
    for path in FileUtils::read_dir(&dir).map_err(|e| format!("Failed to read chart dialog history dir: {}", e))? {
        if path.is_dir() {
...
pub fn cmd_save_chart_chat_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&chat_path, content).map_err(|e| format!("Failed to save chart chat content: {}", e))?;
    let config_path = session_dir.join("config.json");
    if FileUtils::path_exists(&config_path) {
...
pub fn cmd_save_chart_terminal_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&terminal_path, content).map_err(|e| format!("Failed to save chart terminal content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_chart_chat_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&chat_path) {
        let content = FileUtils::read_file_to_string(&chat_path).map_err(|e| format!("Failed to read chart chat content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_load_chart_terminal_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&terminal_path) {
        let content = FileUtils::read_file_to_string(&terminal_path).map_err(|e| format!("Failed to read chart terminal content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
...
pub fn cmd_save_chart_task_content(session_id: &str, content: &str) -> Result<(), String> {
    ...
    FileUtils::write_file_string(&task_path, content).map_err(|e| format!("Failed to save chart task content: {}", e))?;
    Ok(())
}
#[tauri::command]
pub fn cmd_load_chart_task_content(session_id: &str) -> Result<Option<String>, String> {
    ...
    if FileUtils::path_exists(&task_path) {
        let content = FileUtils::read_file_to_string(&task_path).map_err(|e| format!("Failed to read chart task content: {}", e))?;
        Ok(Some(content))
    } else {
        Ok(None)
    }
