...
export function useMapSession(
    ...
    useEffect(() => {
        if (
            ...
            if (currentDomain !== SessionDomain.Map) {
                console.debug(
                    `[useMapSession] Skipping save - current domain is "${currentDomain}", not "Map"`
                );
                return;
            }
            const saveTimer = setTimeout(() => {
                const tasksMap = taskManager.getTasksBySession(currentSessionId, SessionDomain.Map);
                ...
                mapSessionCommands.saveChatContent(currentSessionId, allMessages).catch(console.error);
                mapSessionCommands.saveTerminalContent(currentSessionId, tasksArray).catch(console.error);
            }, 500);
                ...
                .then(list => {
                    if (list.length > 0) {
                        const sorted = list.sort((a, b) => {
                            const aTs = parseInt(a.session_id.replace("map_session_", "")) || 0;
                            const bTs = parseInt(b.session_id.replace("map_session_", "")) || 0;
                            return bTs - aTs;
                        });
                        const sessionId = sorted[0].session_id;
                        setCurrentSessionId(sessionId);
                        Promise.all([
                            mapSessionCommands.loadChatContent(sessionId),
                            mapSessionCommands.loadTerminalContent(sessionId)
                            ...
                            taskManager.loadSessionData(sessionId, terminalContent || [], userMessages, assistantMessages, SessionDomain.Map);
                            setIsLoading(false);
                        }).catch(() => {
                            setIsLoading(false);
                        });
    ...
    const handleSendMessage = useCallback(async (
        ...
        if (finalSessionId && !finalSessionId.startsWith("pending_") &&
            ...
            console.error(
                `[useMapSession] Invalid session ID "${finalSessionId}" - does not belong to Map domain`
            );
            return;
        }
        if (finalSessionId && finalSessionId.startsWith("pending_")) {
        ...
        try {
            ...
            taskManager.addTaskToSession(finalSessionId, newTask, SessionDomain.Map);
        } catch (error) {
    ...
    const handleSwitchSession = useCallback(async (sessionId: string) => {
        ...
        if (currentSessionId && !currentSessionId.startsWith("pending_") && !currentSessionId.startsWith("temp_") && hasData) {
            try {
                ...
                await mapSessionCommands.saveChatContent(currentSessionId, allMessages).catch(console.error);
                await mapSessionCommands.saveTerminalContent(currentSessionId, tasksArray).catch(console.error);
            } catch (error) {
        ...
        if (!hasTargetData) {
            ...
            const terminalContent = await mapSessionCommands.loadTerminalContent(sessionId);
            ...
            taskManager.loadSessionData(sessionId, tasks, userMessages, assistantMessages, SessionDomain.Map);
        } else {
