...
import { ScheduledTask, FixedScheduleConfig, IntervalScheduleConfig } from "./types";
...
const ScheduledTasksForm: React.FC<ScheduledTasksFormProps> = ({
  ...
  const [scheduleType, setScheduleType] = useState<'fixed' | 'interval'>(task?.schedule_type || "fixed");
  const [actionType, setActionType] = useState<'naturallanguage' | 'skillfile'>(task?.action_type || "naturallanguage");
  ...
  const [fixedFrequency, setFixedFrequency] = useState<'daily' | 'weekly' | 'monthly' | 'once'>(
    (task?.schedule_config.type === "fixed" ? task.schedule_config.config.frequency : "daily") as any
  );
  const [fixedTime, setFixedTime] = useState(
    (task?.schedule_config.type === "fixed" ? task.schedule_config.config.time : "09:00") || "09:00"
  );
  const [fixedWeekDays, setFixedWeekDays] = useState<number[]>(
    (task?.schedule_config.type === "fixed" ? task.schedule_config.config.day_of_week : [1]) || [1]
  );
  const [fixedMonthDays, setFixedMonthDays] = useState<number[]>(
    (task?.schedule_config.type === "fixed" ? task.schedule_config.config.day_of_month : [1]) || [1]
  );
  const [fixedDate, setFixedDate] = useState(
    (task?.schedule_config.type === "fixed" ? task.schedule_config.config.date : "") || ""
  );

  // Interval schedule state
  const [intervalUnit, setIntervalUnit] = useState<'second' | 'minute' | 'hour' | 'day'>(
    (task?.schedule_config.type === "interval" ? task.schedule_config.config.unit : "hour") as any
  );
  const [intervalValue, setIntervalValue] = useState(
    (task?.schedule_config.type === "interval" ? task.schedule_config.config.value : 1) || 1
  );
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith(".md") && !file.name.endsWith(".skill.md")) {
      showToast(ToastType.ERROR, t("scheduled.invalidFile") || "请上传 .md 或 .skill.md 文件");
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!name.trim()) {
      showToast(ToastType.WARNING, t("scheduled.taskNameRequired") || "请输入任务名称");
      return;
    }

    if (actionType === "naturallanguage") {
      if (!naturalLanguage.trim()) {
        showToast(ToastType.WARNING, t("scheduled.taskDescriptionRequired") || "请输入任务描述");
        return;
      }
    } else {
      if (!skillContent) {
        showToast(ToastType.WARNING, t("scheduled.skillFileRequired") || "请上传 SKILL.md 文件");
        return;
      }
    }
    ...
    if (scheduleType === "fixed") {
      if (!fixedTime) {
        showToast(ToastType.WARNING, t("scheduled.timeRequired") || "请选择执行时间");
        return;
      }
      if (fixedFrequency === "once" && !fixedDate) {
        showToast(ToastType.WARNING, t("scheduled.dateRequired") || "请选择执行日期");
        return;
      }
    } else {
      if (intervalValue < 1) {
        showToast(ToastType.WARNING, t("scheduled.intervalValueInvalid") || "间隔值必须大于0");
        return;
      }
    }
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "600px", margin: "0 auto", width: "100%" }}>
      ...
      {scheduleType === "fixed" && (
        <>
          <div style={{ marginBottom: "20px" }}>
            <label style={labelStyle}>{t("scheduled.frequency") || "执行频率"}</label>
            <select
              style={inputStyle}
              value={fixedFrequency}
              onChange={(e) => setFixedFrequency(e.target.value as any)}
            >
              <option value="daily">{t("scheduled.frequencyDaily") || "每天"}</option>
              <option value="weekly">{t("scheduled.frequencyWeekly") || "每周"}</option>
              <option value="monthly">{t("scheduled.frequencyMonthly") || "每月"}</option>
              <option value="once">{t("scheduled.frequencyOnce") || "单次"}</option>
            </select>
          </div>

          {fixedFrequency === "weekly" && (
            <div style={{ marginBottom: "20px" }}>
              <label style={labelStyle}>{t("scheduled.weekDays") || "选择星期"}</label>
          ...
          {fixedFrequency === "monthly" && (
            <div style={{ marginBottom: "20px" }}>
              <label style={labelStyle}>{t("scheduled.monthDays") || "选择日期"}</label>
          ...
          {fixedFrequency === "once" && (
            <div style={{ marginBottom: "20px" }}>
              <label style={labelStyle}>{t("scheduled.date") || "执行日期"}</label>
              <input
                type="date"
                style={inputStyle}
                value={fixedDate}
          ...
          <div style={{ marginBottom: "20px" }}>
            <label style={labelStyle}>{t("scheduled.time") || "执行时间"}</label>
            <input
              type="time"
      ...
      {scheduleType === "interval" && (
        <div style={{ display: "flex", gap: "12px", marginBottom: "20px" }}>
          <div style={{ flex: 1 }}>
            <label style={labelStyle}>{t("scheduled.intervalValue") || "间隔数值"}</label>
            <input
              type="number"
              style={inputStyle}
              value={intervalValue}
          ...
          <div style={{ flex: 1 }}>
            <label style={labelStyle}>{t("scheduled.intervalUnit") || "间隔单位"}</label>
            <select
              style={inputStyle}
              value={intervalUnit}
              onChange={(e) => setIntervalUnit(e.target.value as any)}
            >
              <option value="second">{t("scheduled.unitSecond") || "秒"}</option>
              <option value="minute">{t("scheduled.unitMinute") || "分钟"}</option>
      ...
      {actionType === "naturallanguage" && (
        <div style={{ marginBottom: "20px" }}>
          ...
          <textarea
            ...
            placeholder={t("scheduled.naturalLanguagePlaceholder") || "例如：每天凌晨2点备份数据库到 /backup 目录"}
            rows={3}
          />
        </div>
      )}
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          {!skillContent ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              <span style={{ fontSize: "13px", color: "var(--text-primary)" }}>📄 {skillFileName}</span>
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                style={{
      ...
      <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end", marginTop: "24px" }}>
        {isEditing && onToggle && (
          <button
            ...
            style={{
              padding: "10px 20px",
              background: task?.enabled ? "rgba(239, 68, 68, 0.1)" : "rgba(16, 185, 129, 0.1)",
          ...
          >
            {task?.enabled ? (t("scheduled.disable") || "禁用") : (t("scheduled.enable") || "启用")}
          </button>
        )}
        {isEditing && onDelete && (
          <button
        ...
        >
          {isSaving ? (t("common.saving") || "保存中...") : (t("settings.save") || "保存")}
        </button>
      </div>
    </div>
  );
