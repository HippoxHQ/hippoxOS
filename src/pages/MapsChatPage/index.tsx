...
const MapsPage: React.FC<MapsPageProps> = ({
  ...
  useEffect(() => {
    ...
    const savedLeftCollapsed = localStorage.getItem(
      "hippox-map-left-collapsed",
    );
    const savedRightCollapsed = localStorage.getItem(
      "hippox-map-right-collapsed",
    );
    const savedLeftWidth = localStorage.getItem("hippox-map-left-width");
  ...
  const saveLeftCollapsed = (collapsed: boolean) => {
    localStorage.setItem("hippox-map-left-collapsed", collapsed.toString());
  };

  const saveRightCollapsed = (collapsed: boolean) => {
    localStorage.setItem("hippox-map-right-collapsed", collapsed.toString());
  };
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    if (dragType.current === "horizontal") {
      ...
      localStorage.setItem("hippox-map-left-width", newWidthPercent.toString());
  ...
  const getRightPanelContent = () => {
    ...
    if (rightTitle === "Map" || rightTitle === "地图") {
      return (
        <div
          style={{
            display: "flex",
