...
export interface HistoryBlockchainChatPanelRef {
  scrollToTop: () => void;
  scrollToBottom: () => void;
  expandAll: () => void;
  collapseAll: () => void;
...
interface HistoryBlockchainChatPanelProps {
...
const HistoryBlockchainChatPanel = forwardRef<HistoryBlockchainChatPanelRef, HistoryBlockchainChatPanelProps>(({ t, onSessionSelect, currentSessionId, isBatchMode = false, selectedIds = new Set(), onToggleSelection }: HistoryBlockchainChatPanelProps, ref) => {
    ...
    try {
      ...
      const sorted = [...list].sort((a, b) => {
        ...
        const getTimestamp = (id: string) => {
          const ts = id.replace("blockchain_session_", "");
  ...
  const handleTogglePin = async (session: DialogSession, e: React.MouseEvent) => {
    ...
    try {
      ...
      await blockchainSessionCommands.updatePinnedBlockchainSessions(session.session_id, newPinned);
  ...
  const handleDelete = async (session: DialogSession, e: React.MouseEvent) => {
    ...
    showDialog(
      ...
      async () => {
        try {
          await blockchainSessionCommands.deleteBlockchainSession(session.session_id);
  ...
  const saveEdit = async (session: DialogSession) => {
    ...
    try {
      await blockchainSessionCommands.updateBlockchainSessionConfig(session.session_id, {
        title: trimmed,
      });
      await loadSessions(true);
      setEditingId(null);
...
HistoryBlockchainChatPanel.displayName = "HistoryBlockchainChatPanel";
export default HistoryBlockchainChatPanel;
