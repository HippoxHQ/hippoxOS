...
const HistoryCodeEditorChatPanel = forwardRef<HistoryCodeEditorChatPanelRef, HistoryCodeEditorChatPanelProps>(({ t, onSessionSelect, currentSessionId, onNewSession, onAllSessionsDeleted, isBatchMode = false, selectedIds = new Set(), onToggleSelection }: HistoryCodeEditorChatPanelProps, ref) => {
  ...
  const handleTogglePin = async (session: DialogSession, e: React.MouseEvent) => {
    ...
    try {
      ...
      await codeEditorSessionCommands.updatePinnedCodeEditorSessions(session.session_id, newPinned);
  ...
  const saveEdit = async (session: DialogSession) => {
    ...
    try {
      await codeEditorSessionCommands.updateCodeEditorSessionConfig(session.session_id, {
        title: trimmed,
      });
      await loadSessions(true);
      setEditingId(null);
