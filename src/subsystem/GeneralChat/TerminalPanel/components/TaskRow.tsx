...
export const TaskRow = forwardRef<HTMLDivElement, TaskRowProps>(({ task, isExpanded, expandedStepParams, filesScrollState, filesScrollRefs, setFilesScrollStates, onToggleExpand, onToggleStepParams, onScrollFilesLeft, onScrollFilesRight, onFileClick, setTasks, t }, ref) => {
  ...
  return (
    <div key={task.task_id} ref={ref} className="task-row">
      ...
      >
        ...
        >
          <span
            ...
            style={{
              ...(task.status === TaskStatusEnum.Failed ? { color: "#ff4444" } : task.status === TaskStatusEnum.Paused ? { color: "#ffa500" } : task.status === TaskStatusEnum.Cancelled ? { color: "#888888" } : {}),
              whiteSpace: "nowrap",
            }}
          >
      ...
      {isExpanded && task.final_output && task.status === TaskStatusEnum.Completed && (
        <>
          <TaskOutput output={getRawOutput(task.task_id, task.session_id) || task.final_output} onCopy={handleCopyOutput} onShowChart={handleShowChart} onShowMap={handleShowMap} taskId={task.task_id} t={t} onFileClick={onFileClick} />
        </>
      )}
      {isExpanded && task.status === TaskStatusEnum.Failed && task.final_output && <TaskError error={(task as any).rawOutput || task.final_output} onCopy={handleCopyOutput} onShowChart={handleShowChart} t={t} />}
