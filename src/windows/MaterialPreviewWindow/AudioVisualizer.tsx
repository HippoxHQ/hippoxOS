...
>(
  (
    { audioUrl, isDark, isPlaying, onPlayStateChange, onTimeUpdate, onLoaded },
    ref,
  ) => {
    ...
    const drawParticles = (
      ...
      stars.forEach((star) => {
        ...
        ctx.fillStyle = isDark
          ? `rgba(180, 220, 255, ${opacity * 0.5})`
          : `rgba(100, 150, 200, ${opacity * 0.3})`;
        ctx.fill();
        if (star.size > 1) {
          ...
          const glowColor = isDark
            ? `rgba(150, 200, 255, ${opacity * 0.1})`
            : `rgba(100, 150, 200, ${opacity * 0.05})`;
      ...
      const glowColor = isDark
        ? `rgba(78, 201, 176, ${0.05 + overall * 0.08})`
        : `rgba(30, 150, 130, ${0.05 + overall * 0.08})`;
      ...
      particles.forEach((particle, i) => {
        const spectrumIndex = Math.floor(
          (i / particles.length) * spectrum.length,
        );
        ...
        if (currentSize > 2) {
          ctx.beginPath();
          ctx.arc(
            particle.x - currentSize * 0.2,
            particle.y - currentSize * 0.2,
            currentSize * 0.3,
            0,
            Math.PI * 2,
          );
      ...
      ctx.fillStyle = isDark
        ? `rgba(255, 255, 255, ${0.3 + overall * 0.5})`
        : `rgba(255, 255, 255, ${0.5 + overall * 0.4})`;
      ctx.fill();
      ctx.shadowBlur = 20;
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        ctx.strokeStyle = isDark
          ? `hsla(180, 80%, 60%, ${ringAlpha})`
          : `hsla(190, 80%, 50%, ${ringAlpha})`;
    ...
    useEffect(() => {
      ...
      const animate = () => {
        ...
        const bass = hasData
          ? dataArray2.slice(0, 10).reduce((a, b) => a + b, 0) / (10 * 255)
          : 0.05 + Math.sin(Date.now() / 3000) * 0.03;
        const mid = hasData
          ? dataArray2.slice(10, 30).reduce((a, b) => a + b, 0) / (20 * 255)
          : 0.05 + Math.cos(Date.now() / 2500) * 0.03;
        const treble = hasData
          ? dataArray2.slice(30, 60).reduce((a, b) => a + b, 0) / (30 * 255)
          : 0.05 + Math.sin(Date.now() / 2000 + 1) * 0.03;
        const dpr = window.devicePixelRatio || 1;
        drawParticles(
          ctx2,
          canvas.width / dpr,
